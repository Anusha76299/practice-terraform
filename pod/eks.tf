...
/* data "aws_iam_policy_document" "karpenter_controller_assume_role_policy" {
  statement {
    ...
    condition {
      ...
      values = ["system:serviceaccountaccount:karpenter:karpenter"]
    }
...
/*resource "null_resource" "demo" {
 provisioner "local-exec" {
     command = "aws eks --region us-east-1 describe-cluster --name demo1 --query cluster.status"
 }
}
...
resource "null_resource" "demo5" {
 ...
 provisioner "local-exec" {
 command = "kubectl delete -f eks.yml"
 }
...
/*resource "null_resource" "demo3" {
 ...
 provisioner "local-exec" {
 command = "kubectl create -f eks.yml"
 }
